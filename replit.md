...
- `scripts/src/paper.ts` (and `scripts/src/zenodoDeposit.ts`) now build at **rubric v0.9.0**: `WEIGHTS` = 15/12/12/15/14/7/25 and `REQUIRED_RUBRIC_VERSION` = "0.9.0". The corpus (eval ids 12, 35–47) was re-scored under v0.9.0 before the deposition; the paper builder gates on every corpus eval being complete@0.9.0. The prior **published PRODUCTION v0.8.0 record** (DOI 10.5281/zenodo.20567720, weights 18/14/14/18/18/8/10) is immutable and unaffected; its v0.8.0 builder constants are recoverable from git history at the pre-bump commit. The v0.9.0 report is now **published to PRODUCTION** (DOI 10.5281/zenodo.20576496) — this record includes the long-form "Appendix A — Full Per-Project Evaluations" (full summary/findings/gaps/recommendations + per-dimension scores per project) added to `scripts/src/paper.ts`. (An earlier v0.9.0 SANDBOX draft 509977, predating the appendix, is superseded.) A future rubric bump again needs its own re-scored corpus + updated constants in both builders.
