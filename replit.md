...
- The rubric is **versioned**: `RUBRIC_VERSION` in `paperPipeline.ts` (currently `"3.0.0"`) is the single source of truth and is stamped onto each evaluation (`rubric_version` column) at scoring time. Bump it whenever you change dimensions (MAJOR), weights (MINOR), or guidepost wording/calibration (PATCH) so scores from different rubrics stay comparable. `null` = scored before versioning existed; historic rows are not backfilled (they get stamped only on rerun).
