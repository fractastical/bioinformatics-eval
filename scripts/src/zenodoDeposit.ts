...
function buildMetadata(): ZenodoMetadata {
  ...
  return {
    ...
    description: [
      ...
      "<p>The central question BioEval asks is whether a simulation accords with good data and reproducibility practice. Six dimensions measure exactly this: Data Disclosure (15%), Dataset Resolvability (12%), Code Availability &amp; Versioning (12%), Code-to-Data Traceability (15%), Simulation Derivation Clarity (14%), and Reproducibility Package Quality (7%). Scores are anchored to verified external signals &mdash; accessions and DOIs resolved against Crossref and DataCite, live repository facts such as a detected license &mdash; rather than the paper's own claims, and the framework does not credit unverifiable assertions or invent unsubstantiated gaps.</p>",
      "<p>A seventh, orthogonal dimension, Information-Theoretic Rigor (25%), scores whether a paper formalizes and quantifies the information content of the system it models &mdash; Shannon entropy, mutual and transfer entropy, channel capacity, communication bit rate &mdash; which is particularly apt for swarm, stigmergy, and synchronization systems where information flow IS the phenomenon. It measures scientific-content rigor, not transparency; topics with no information-theoretic dimension are scored at a neutral baseline (~50 on the 0&ndash;100 scale) so non-applicability neither rewards nor punishes. In rubric v0.9.0 it carries the single largest weight of the seven dimensions, an intentional elevation that reflects that for this corpus the quantification of information flow is central to the science rather than a peripheral concern. The overall score is the weighted mean of the seven dimensions.</p>",
      "<p>The bundled report applies the rubric (version 0.9.0) to fourteen insect-swarm and agent-based simulation projects, reporting the raw seven-dimension scores including per-paper information-theoretic rigor. Its headline finding is that transparency is uneven and reproducibility packaging consistently weak, while information-theoretic rigor is systematically low even across projects whose entire subject is communication and coordination.</p>",
      "<p>Version 0.9.0 is a pre-1.0 release: the tool and its evaluation rubric are co-versioned and the rubric is still under reviewer validation. It revises the dimension weights of the earlier 0.8.0 release, elevating Information-Theoretic Rigor from 10% to 25%. Built as a pnpm monorepo: React + Vite frontend, Express 5 API, PostgreSQL + Drizzle ORM, contract-first OpenAPI/Orval codegen.</p>",
    ].join("\n"),
    creators: [creator],
    keywords: [
      "reproducibility",
...
- 7 weighted dims (0-100): Data Disclosure 15%, Dataset Resolvability 12%, Code Availability & Versioning 12%, Code-to-Data Traceability 15%, Simulation Derivation Clarity 14%, Reproducibility Package Quality 7%, Information-Theoretic Rigor 25%. Overall = weighted average. Information-Theoretic Rigor is the single largest weight in v0.9.0 (an intentional elevation from 10% in v0.8.0); "orthogonal" describes WHAT it measures (scientific-content rigor, independent of transparency), not a low importance.
